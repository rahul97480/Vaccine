...
function startSound($json){
   ...
   for($i=0;$i<$size;$i++){
      ...
      if(!array_key_exists('vaccine_fees',$json['centers'][$i])){
         for($j=0;$j<$size_session;$j++){
             if($json['centers'][$i]['sessions'][$j]['min_age_limit'] == 18){
               if($json['centers'][$i]['sessions'][$j]['available_capacity_dose1'] > 1 ){
...
// $result2 = callAPI('GET', 'https://cdn-api.co-vin.in/api/v2/appointment/sessions/public/calendarByDistrict?district_id=83&date=24-06-2021');
